...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('front/css/page-profile.css') }}" />
...
<body>
    ...
    <script src="{{ asset('front/js/ds.js') }}"></script>
    <!-- Script JavaScript -->
        ...
        function autofill() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $("#skl").val(response.sekolah)
                    $("#nama").val(response.nama)
                    $("#tanggal").val(response.tanggal)
                    $("#kab_kota").val(response.kab_kota)
                    $("#kec").val(response.kec)
                    $("#ds").val(response.ds)
                }
            });
        }
    ...
    <script>
        $(document).ready(function() {
            $('#uploadZipForm').on('submit', function(e) {
                ...
                progressContainer.show(); // Menampilkan progress bar saat formulir diajukan
                $.ajax({
                    xhr: function() {
                        ...
                        xhr.upload.addEventListener("progress", function(evt) {
                            if (evt.lengthComputable) {
                                ...
                                progressBar.text(percentComplete.toFixed(2) +
                                    '%'
                                ); // Menampilkan persentase progres di dalam progress bar
                            }
                        }, false);
                        return xhr;
                    },
                    url: '{{ route('extract-zip') }}',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(response) {
                        ...
                        // alert('Data berhasil diekstrak', 'success');
                        toastr.success('Data berhasil diekstrak.', 'success');
                    },
                    error: function(xhr, status, error) {
                        progressContainer.hide();
                        alert('Gagal mengunggah dan mengekstrak file zip', 'error');
                    }
                });
            });
        });
